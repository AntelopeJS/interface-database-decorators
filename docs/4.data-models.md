...
During data conversions and database operations, modifier events are automatically triggered at specific points in the data lifecycle. These events allow modifiers to perform validation, logging, and other side effects:

| Conversion Method | Event Triggered | When                                    |
|-------------------|-----------------|-----------------------------------------|
| `fromPlainData()` | `fromPlainData` | After creating instance from plain data |
| `toPlainData()`   | `toPlainData`   | After converting instance to plain data |
| `fromDatabase()`  | `fromDatabase`  | After reading from database             |
| `toDatabase()`    | `toDatabase`    | Before writing to database              |

Additionally, the model's `insert()` and `update()` methods trigger their respective events:

```typescript
// The `insert` event is triggered before inserting
await userModel.insert(userData);

// The `update` event is triggered before updating
await userModel.update(userId, updates);
```

For more information on creating modifiers that use these events, see the [Modifier Events section in Table Modifiers](./3.table-modifiers.md#modifier-events).
