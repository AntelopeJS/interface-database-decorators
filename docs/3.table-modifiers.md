...
The wildcard key `'*'` allows you to get or set all translations at once, useful for imports, exports, or bulk updates:

```typescript
// Get all translations for a field
const allNames = product.localize('*').name;
// Returns: { en: 'Premium Headphones', fr: 'Casque Premium', es: 'Auriculares Premium' }
...
const exportData = {
  name: product.localize('*').name,
  description: product.localize('*').description
};
```
...
### Container-Based Modifiers

For modifiers that need to store multiple values in a key-value structure (like `LocalizationModifier`), you can extend `ContainerModifier`. This base class provides built-in support for the wildcard key `'*'` for bulk operations, allowing users to get or set all values at once.

Common use cases for container-based modifiers include multi-language content, environment-specific settings, feature flags, or versioned data.

## Modifier Events

Modifiers can hook into the data lifecycle by implementing event handler methods. These events are triggered during data conversions and database operations:

| Event           | Triggered When                          |
|-----------------|-----------------------------------------|
| `fromPlainData` | After converting plain data to instance |
| `toPlainData`   | After converting instance to plain data |
| `fromDatabase`  | After reading from database             |
| `toDatabase`    | Before writing to database              |
| `insert`        | Before inserting a new record           |
| `update`        | Before updating an existing record      |

```typescript
// Automatic timestamp modifier
class TimestampModifier extends Modifier<unknown, []> {
...
// Validation modifier
class ValidationModifier extends Modifier<unknown, []> {
  public fromPlainData(instance: any, field: string) {
...
Modifiers can be event-only (without `lock`/`unlock` methods) for side-effects like logging, or combine events with transformations for validation alongside data conversion.
